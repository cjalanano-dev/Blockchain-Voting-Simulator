    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-auto space-y-2 pr-1 custom-scroll">
        {pendingVotes.length === 0 && (
          <p className="text-sm text-neutral-500 dark:text-neutral-400">No votes waiting. Cast some above.</p>
        )}
        {pendingVotes.map(v => (
          <div key={v.id} className="rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800/70 px-3 py-2 text-xs flex justify-between items-center">
            <span className="font-mono text-neutral-600 dark:text-neutral-400">{v.id.slice(0,8)}</span>
            <span className="font-medium text-neutral-800 dark:text-neutral-200">{v.candidate}</span>
            <span className="text-neutral-400 dark:text-neutral-500">{new Date(v.timestamp).toLocaleTimeString()}</span>
          </div>
        ))}
      </div>
      <div className="pt-4 mt-4 border-t border-neutral-200 dark:border-neutral-700">
        ...
        {lastHash && !mining && (
          <div className="mt-3 text-[10px] font-mono break-all text-neutral-500 dark:text-neutral-400">
